...
interface CreateLoopModalProps {
  isOpen: boolean
  onClose: () => void
  onCreate: (data: { title: string; visibility: string; growthEnabled: boolean }) => Promise<void>
}
...
export function CreateLoopModal({ isOpen, onClose, onCreate }: CreateLoopModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onCreate({ title: title.trim(), visibility, growthEnabled })
      // Reset form
      setTitle('')
      ...
      console.error('[CreateLoopModal] Error creating loop:', error)
    } finally {
          ...
          >
            ...
            >
              ...
              <form onSubmit={handleSubmit}>
                {/* Title Input */}
                <div style={{ marginBottom: tokensV2.spacing[24] }}>
                  <label style={{
                    display: 'block',
